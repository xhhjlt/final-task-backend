...
export const createBoard = async (req: Request, res: Response) => {
  ...
  const bodyError = checkBody(req.body, ['title', 'owner', 'users', 'description'])
  if (bodyError) {
    return res.status(400).send(createError(400, "bad request: " + bodyError));
  }

  const { title, owner, users, description } = req.body;
  try {
...
export const updateBoard = async (req: Request, res: Response) => {
  ...
  try {
    const updatedBoard = await boardService.updateBoard(req.params['boardId'], { title, owner, users, description }, guid, initUser);
